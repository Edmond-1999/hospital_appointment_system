...
def view_patient(patient_id: UUID, service: DoctorService = Depends(get_doctor_service)):
    return service.view_patient(patient_id)


@router.patch("/{doctor_id}/appointments/{appointment_id}/status")
def change_status(appointment_id: UUID,status: str,service: DoctorService = Depends(get_doctor_service)):
